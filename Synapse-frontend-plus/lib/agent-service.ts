...
// 获取用户的助理列表
export async function getUserAgents(params?: Partial<GetAgentsParams>): Promise<ApiResponse<AgentListVO>> {
  try {
...
// 获取已发布的助理列表
export async function getPublishedAgents(name?: string): Promise<ApiResponse<AgentListVO>> {
  try {
    const queryParams: Record<string, any> = {}

    // 添加关键词搜索参数（后端使用 keyword 而不是 name）
    if (name) {
...
export async function createAgentVersion(
  ...
  try {
    const url = buildApiUrl(API_ENDPOINTS.CREATE_AGENT_VERSION(agentId))

    console.log(`Creating agent version at: ${url}`)
    ...
    if (!response.ok) {
      throw new Error(`创建助理版本失败: ${response.status}`)
    }

    const data = await response.json()
    return data
  } catch (error) {
    console.error("创建助理版本错误:", error)
    return {
      code: 500,
      message: error instanceof Error ? error.message : "未知错误",
...
export async function publishAgentVersion(
  ...
  try {
    const url = buildApiUrl(API_ENDPOINTS.PUBLISH_AGENT_VERSION(agentId, versionId))
