...
import { getSession, getSessionMessages } from "@/lib/api-services"
import type { Session, MessageItem } from "@/types/conversation"
...
export function ChatPanel({ conversationId }: ChatPanelProps) {
  ...
  // 将后端消息格式转换为前端消息格式
  const convertMessageItemToMessage = (item: MessageItem): Message => {
    return {
      id: item.id,
    ...
    const fetchSessionAndMessages = async () => {
      try {
        ...
        const [sessionResponse, messagesResponse] = await Promise.all([
          ...
          getSessionMessages(conversationId, { page: 1, pageSize: 100 }),
        ])
        ...
        if (sessionResponse.code === 200 && sessionResponse.data) {
          ...
          // 处理API返回的错误
          const errorMessage = sessionResponse.message || "获取会话详情失败"
          ...
          sessionTitle = fallbackSession.title
        ...
        if (messagesResponse.code === 200 && messagesResponse.data) {
          const messageItems = messagesResponse.data.messages || []
          
          if (messageItems.length > 0) {
            // 如果有历史消息，转换为前端格式并显示
            const convertedMessages = messageItems.map(convertMessageItemToMessage)
            ...
            setMessages([
              {
                ...
                role: "assistant",
                content: `欢迎来到 "${sessionTitle}" 会话。有什么可以帮助您的吗？`,
              },
            ])
          }
        } else {
        ...
        console.error("获取会话详情或消息错误:", error)
