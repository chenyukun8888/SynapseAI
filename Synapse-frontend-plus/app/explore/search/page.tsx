...
export default function SearchPage() {
  ...
  // 根据类型过滤助理
  const filteredAgents = (agents && Array.isArray(agents) ? agents : []).filter((agent) => {
    if (activeTab === "all") return true
    if (activeTab === "chat") return agent.type === "CHAT_ASSISTANT"
    if (activeTab === "functional") return agent.type === "FUNCTIONAL_AGENT"
    return true
  })
...
// 渲染助理列表
function renderAgentList(agents: AgentListItem[], loading: boolean, error: string | null) {
  if (loading) {
    return (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    <Badge variant="outline" className="text-[10px]">
                      {agent.type === "CHAT_ASSISTANT" ? "聊天助理" : "功能性助理"}
                    </Badge>
                    <Badge variant="outline" className="text-[10px]">
                      v{agent.version}
                    </Badge>
