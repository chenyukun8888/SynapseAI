...
import type { Agent, AgentListItem } from "@/types/agent"
...
export default function StudioPage() {
    ...
    async function fetchAgents() {
      try {
        ...
        if (response.code === 200) {
          // 后端返回的是 AgentListVO，包含 agents 数组
  ...
  const handleDeleteAgent = async () => {
    ...
    try {
      setIsDeleting(true)
      const response = await deleteAgent(agentToDelete.agentId)
      ...
      if (response.code === 200) {
        ...
        // 更新列表，移除已删除的助理
        setAgents(agents.filter((agent) => agent.agentId !== agentToDelete.agentId))
      } else {
        toast({
          title: "删除失败",
          description: response.message,
  ...
  const handleToggleStatus = async (agent: AgentListItem) => {
    try {
      setIsTogglingStatus(agent.agentId)
      const response = await toggleAgentStatus(agent.agentId)
      ...
      if (response.code === 200) {
        ...
        // 更新列表中的助理状态
        // 注意：toggleAgentStatus 返回的是 Agent，但列表中是 AgentListItem
        // 需要重新获取列表或更新对应字段
        const updatedAgents = agents.map((a) => {
          if (a.agentId === agent.agentId) {
            return {
              ...a,
              status: response.data.enabled ? "ENABLED" : "DISABLED",
            }
          }
          return a
        })
  ...
  const getAgentTypeText = (type: string) => {
    return type === "CHAT_ASSISTANT" ? "聊天助理" : "功能性助理"
  }
            ...
            <Card key={agent.agentId}>
              <CardHeader className="pb-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem asChild>
                        <Link href={`/studio/edit/${agent.agentId}`}>
                          <Edit className="mr-2 h-4 w-4" />
                          编辑
                        </Link>
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => handleToggleStatus(agent)}>
                        {isTogglingStatus === agent.agentId ? (
                          <>
                ...
                <CardDescription className="text-xs flex items-center gap-2 mt-1">
                  <span>{getAgentTypeText(agent.type)}</span>
                  <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                  <span>{agent.status === "ENABLED" ? "已启用" : "已禁用"}</span>
                  <span className="w-1 h-1 rounded-full bg-gray-300"></span>
              ...
              <CardFooter className="flex justify-between">
                ...
                <Button size="sm" asChild disabled={agent.status !== "ENABLED"} variant={agent.status === "ENABLED" ? "default" : "outline"}>
